...
KNN, unlike logistic regression, are particularly useful for classification tasks due to their ability to identify the closest training samples (neighbors) in feature space relative to a given test instance and using their labels for predictions. However, KNN can be highly sensitive to outliers, and may lead to overfitting as a result of noises. Normalizing the dataset during the initial analysis stage is crucial for ensuring equal weights for each predictor in KNN calculations.

We plan to employ two KNN models: one with an initial k value equal to the square root of the training set size, which is a common practice; and another one through cross validation tuning to determine the optimal k value for the model.
...
We will utilize the square root of the number of rows in our training set as an initial value for the parameter k. Subsequently, we will apply grid search to fine-tune our KNN model and evaluate its potential for improving accuracy and overall fit.
...
We will execute a grid search with 10-fold cross validation to identify the optimal k value and subsequently use that tuned KNN model for prediction.
...
According to the results, the optimal value of k is ``r best_k``.
